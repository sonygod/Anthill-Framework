...
using Lambda;
using Reflect;
class AntEntity extends AntBasic  implements IBubbleEventHandler {
	@:isVar public var depth(get, never) : Int;
	@:isVar public var isGroup(get, never) : Bool;
	@:isVar public var isScrolled(get, set) : Bool;
	 ...
	 */
	static inline var ASCENDING : Int = -1;
	/**

	 * Констатна для определения способа сортировки по убыванию.

	 */
	static inline var DESCENDING : Int = 1;
	//---------------------------------------
	// PUBLIC VARIABLES
	//---------------------------------------
	/**
	 ...
	 */
	public var autoReviveChildren : Bool;
	/**
	 ...
	 */
	static public var DEPTH_ID : Int = 0;
	//---------------------------------------
	// CONSTRUCTOR
	//---------------------------------------
	/**
	...
	override public function destroy() : Void {
		...
		if(children != null)  {
			...
			var i : Int = 0;
			while(i < numChildren) {
				entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
				if(entity != null)  {
					entity.destroy();
				}
			}
	...
	override public function debugDraw(aCamera : AntCamera) : Void {
		...
		drawer.moveTo(Std.int(p.x), Std.int(p.y));
		...
		drawer.lineTo(Std.int(p.x), Std.int(p.y), AntColor.LIME);
		...
		drawer.drawRect(Std.int(p.x), Std.int(p.y), bounds.width, bounds.height, AntColor.FUCHSIA);
		...
		drawer.drawAxis(Std.int(p.x), Std.int(p.y), AntColor.AQUA);
	 ...
	 */
	public function reset(aX : Float = 0, aY : Float = 0, aAngle : Float = 0) : Void {
		x = aX;
		y = aY;
		angle = aAngle;
		if(parent != null)  {
	 ...
	 */
	public function remove(aEntity : AntEntity, aSplice : Bool = false) : AntEntity {
		if(children == null)  {
			return aEntity;
		}
	 ...
	 */
	public function contains(aEntity : AntEntity) : Bool {
		if(children == null)  {
			return false;
		}
	 ...
	 */
	public function recycle(aClass : Class<Dynamic> = null) : AntEntity {
		...
		entity = try cast(Type.createInstance(aClass, []), AntEntity) catch(e:Dynamic) null;
		return ((Std.is(entity, AntEntity))) ? add(entity) : null;
	}
	 ...
	 */
	public function removeAll(aDestroy : Bool = true) : Void {
		if(children == null)  {
			return;
		}
		var entity : AntEntity;
	 ...
	 */
	public function getAvailable(aClass : Class<Dynamic> = null) : AntEntity {
		if(children == null)  {
			return null;
		}
		var entity : AntEntity;
		...
		while(i < numChildren) {
			entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
			if(entity != null && !entity.exists && ((aClass == null) || (Std.is(entity, aClass))))  {
				return entity;
			}
		}
	 ...
	 */
	public function getExtant(aClass : Class<Dynamic> = null) : AntEntity {
		if(children == null)  {
			return null;
		}
		var entity : AntEntity;
		...
		while(i < numChildren) {
			entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
			if(entity != null && ((aClass == null) || (Std.is(entity, aClass))))  {
				return entity;
			}
		}
	 ...
	 */
	public function getAlive(aClass : Class<Dynamic> = null) : AntEntity {
		if(children == null)  {
			return null;
		}
		var entity : AntEntity;
		...
		while(i < numChildren) {
			entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
			if(entity != null && entity.exists && entity.alive && ((aClass == null) || (Std.is(entity, aClass))))  {
				return entity;
			}
		}
	 ...
	 */
	public function getDead(aClass : Class<Dynamic> = null) : AntEntity {
		if(children == null)  {
			return null;
		}
		var entity : AntEntity;
		...
		while(i < numChildren) {
			entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
			if(entity != null && !entity.alive && ((aClass == null) || (Std.is(entity, aClass))))  {
				return entity;
			}
		}
	 ...
	 */
	public function getRandom(aClass : Class<Dynamic> = null, aExistsOnly : Bool = true) : AntEntity {
		if(children == null)  {
			return null;
		}
		var entity : AntEntity;
		var exists : Array<Dynamic> = [];
		var i : Int = 0;
		while(i < numChildren) {
			entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
			if(entity != null)  {
				if(aExistsOnly && entity.exists && ((aClass == null) || (Std.is(entity, aClass))))  {
					exists[exists.length] = entity;
				}

				else if(!aExistsOnly && ((aClass == null) || (Std.is(entity, aClass))))  {
					exists[exists.length] = entity;
				}
			}
		}

		entity = try cast(exists[AntMath.randomRangeInt(0, exists.length)], AntEntity) catch(e:Dynamic) null;
		i = 0;
	 ...
	 */
	public function queryByTag(aTag : Int, aResult : Array<Dynamic> = null) : Array<Dynamic> {
		if(children == null)  {
			return null;
		}
		if(aResult == null)  {
	 ...
	 */
	public function setAll(aVariableName : String, aValue : Dynamic, aRecurse : Bool = true) : Void {
		var entity : AntEntity;
		var i : Int = 0;
		while(i < numChildren) {
			entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
			if(entity != null)  {
				...
				entity.setField(aVariableName, aValue);
			}
		}
	 ...
	 */
	public function callAll(aFunctionName : String, aArgs : Array<Dynamic> = null, aRecurse : Bool = true) : Void {
		var entity : AntEntity;
		var i : Int = 0;
		while(i < numChildren) {
			entity = try cast(children[i++], AntEntity) catch(e:Dynamic) null;
			if(entity != null)  {
				...
				//if(Std.is(entity[aFunctionName], Function))  {
				
				if(entity.field(aFunctionName).isFunction()){
					entity.field(aFunctionName).apply(this, aArgs);
				}
			}
		}
	 ...
	 */
	public function hitTest(aX : Float, aY : Float) : Bool {
		var n : Int = vertices.length;
		var res : Bool = false;
		var i : Int = 0;
		var j : Int = n - 1;
		while(i < n) {
			if(((vertices[i].y > aY) != (vertices[j].y > aY)) && (aX < (vertices[j].x - vertices[i].x) * (aY - vertices[i].y) / (vertices[j].y - vertices[i].y) + vertices[i].x))  {
				res = !res;
			}
			j = i++;
		}
	 ...
	 */
	public function hitTestPoint(aPoint : AntPoint) : Bool {
	 ...
	 */
	public function onScreen(aCamera : AntCamera = null) : Bool {
		if(aCamera == null)  {
	 ...
	 */
	function toScreenPosition(aX : Float, aY : Float, aCamera : AntCamera = null, aResult : AntPoint = null) : AntPoint {
		if(aResult == null)  {
	 ...
	 */
	public function hurt(aDamage : Float) : Bool {
		health -= aDamage;
		if(health <= 0)  {
			kill();
			return true;
	...
	public function updateBounds() : Void {
		...
		if(globalAngle == 0 && (!_oldPosition.equal(globalX, globalY) || !_oldSize.equal(width, height) || !_oldScale.equal(scaleX, scaleY)))  {
			calcBounds();
		}

		else if(_oldAngle == globalAngle && !_oldPosition.equal(globalX, globalY) && _oldSize.equal(width, height) && _oldScale.equal(scaleX, scaleY))  {
			moveBounds();
		}
	 ...
	 */
	public function onEventBubbled(aEvent : IEvent) : Bool {
		/*
	 ...
	 */
	function drawChildren(aCamera : AntCamera) : Void {
		if(children != null)  {
			var entity : AntEntity;
			var i : Int = 0;
			while(i < numChildren) {
	 ...
	 */
	function moveBounds() : Void {
		var mx : Float = globalX - _oldPosition.x;
		var my : Float = globalY - _oldPosition.y;
		bounds.x += mx;
		bounds.y += my;
		var p : AntPoint;
		var i : Int = 0;
	 ...
	 */
	function locate(aX : Float, aY : Float, aAngle : Float) : Void {
		var rad : Float = aAngle / 180 * Math.PI;
		var px : Float = x;
		// + origin.x;
		var py : Float = y;
		// + origin.y;
		var dx : Float = px * Math.cos(rad) - py * Math.sin(rad);
		var dy : Float = px * Math.sin(rad) + py * Math.cos(rad);
		globalX = aX + dx;
		globalY = aY + dy;
	 ...
	 */
	function sortHandler(aEntity1 : AntEntity, aEntity2 : AntEntity) : Int {
		if(aEntity1 == null)  {
			return _sortOrder;
		}
		...
		if(aEntity1.field(_sortIndex) < aEntity2.field(_sortIndex))  {
			return _sortOrder;
		}

		else if(aEntity1.field(_sortIndex) > aEntity2.field(_sortIndex))  {
			return -_sortOrder;
		}
		return 0;
	}
	 ...
	 */
	function get_isScrolled() : Bool {
		return ((scrollFactor.x == 0 && scrollFactor.y == 0)) ? false : true;
	}

	function set_isScrolled(value : Bool) : Bool {
